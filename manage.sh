...
###################################################
#Instructions
For Windows user, import openvpn config file. After that, enter your username, password and private key via OpenVPN UI

For Linux machines, need to create a folder with the unique name and put your certificate .ovpn extension file, create two files on the same location to save your username, password and passphrase
$ sudo mkdir /etc/openvpn/openvpn-2fa
$ sudo cd /etc/openvpn/openvpn-2fa
$ sudo cp /home/username/Downloads/openvpn-client.ovpn /etc/openvpn/openvpn-2fa/ (copy it from download location)
$ sudo vim client.pass (first put username[VPN usename], password[VPN password] and save it)
e.g.
myvpn.user
password
$ sudo vim client.priv (only add your passowrd and save it)
e.g.
password
$ sudo chown $(whoami):$(whoami) client.pass && chmod 600 client.pass && chown $(whoami):$(whoami) client.priv && chmod 600 client.priv
